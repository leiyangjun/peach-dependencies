...
本仓库为 **Maven BOM 工程**，自身不产生可运行应用；通过 `dependencyManagement` 集中 **import** Spring Boot / Spring Cloud / Spring Cloud Alibaba 官方 BOM，并锁定 Druid、MyBatis、PageHelper、Jasypt、Fastjson2、EasyExcel、Redisson、Hutool、MapStruct、Lombok、Commons、ZXing 等常用组件版本。业务父 POM 只需 **import** 本工程即可省略大量 `<version>`。

## 功能说明

- 统一 **JDK 21**、**Spring Boot 4.0.x**、**Spring Cloud 2025.1.x**、**SCA 2025.1.0.0** 版本线。
- 与 **Sonatype Central** 发布流程对齐（`central-publishing-maven-plugin`、`release-sign` Profile、sources/javadoc）。
- 注释与 `pom.xml` 中说明 **Redis/RabbitMQ/Kafka** 等由 Spring Boot BOM 托管，引用对应 Starter 时不写版本即可。

## 使用说明（构建）

在**本工程根目录**（含 `pom.xml`）执行：
...
**环境变量**：本 BOM 工程无运行时环境变量；发布 Central 需本机 Maven `settings.xml` 中配置 `central` 凭据，可选 GPG（`-Prelease-sign`）。

---

## 开发约定（与 Peach 体系对齐）

- **父 POM**：多数 sibling 工程使用 `spring-boot-starter-parent`，再通过 `dependencyManagement` **import** 本 BOM（与「单父 POM」二选一，当前仓库为前者）。
- **版本号**：与 `peach-common-start`、`peach-gateway` 等模块中 `peach-dependencies.version` 属性保持一致（常为 `0.0.1-SNAPSHOT`）。
- **注释风格**：`pom.xml` 中保留「创作日期 / 作者」类说明块，升级依赖时同步更新下文「版本管理说明」表。
...
本仓库使用 **单一 `origin`**：**拉取** 默认来自 **GitHub**；**推送** 时 Git 会 **依次推送到 GitHub 与 Gitee**（两条 `pushurl`）。

| 用途 | 地址 |
|------|------|
| **fetch（默认）** | `git@github.com:leiyangjun/peach-dependencies.git` |
| **push** | 同上 + `git@gitee.com:leiyangjun/peach-dependencies.git` |

日常提交后 **只需一条推送命令**（分支名按实际，如 `main` 或 `master`）：
...
之后可直接 `git push`，会同时更新 GitHub 与 Gitee（若某一端失败，整次 `push` 会报错，需按提示修复该端权限或网络后重试）。

在工程根目录执行 `git remote -v`、`git config --get-all remote.origin.pushurl` 可核对配置。
